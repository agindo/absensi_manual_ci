	...
	public function lap1(){
		if(isset($_POST['lap1_submit'])){
			...
			if(isset($_POST['cetak_lap1_submit'])){
				...
				$pdf->cell(30, 7, '', 0, 0, 'C');
				$pdf->Cell(215,6,'LAPORAN ABSENSI Per HARI',0,1,'R');
				$pdf->SetFont('Arial','',11);
				$pdf->cell(30, 7, '', 0, 0, 'C');
				$pdf->Cell(215,6,'Hari, Tanggal : '.$lata['hari'].", ".$lata['tgl']." ".$lata['bln']." ".$lata['thn'],0,1,'L');
				...
				$pdf->cell(100, 7, 'Nama Karyawan', 1, 0);
				$pdf->cell(50, 7, 'Shift I', 1, 0, 'C');
				$pdf->cell(50, 7, 'Shift II', 1, 1, 'C');
				...
				foreach ($data->result() as $value) {
					$pdf->cell(30, 7, '', 0, 0, 'C');
					$pdf->cell(15, 7, $no, 1, 0, 'C');
					$pdf->cell(100, 7, $value->nama_karyawan, 1, 0);
					$pdf->cell(50, 7, $value->jam_masuk." - ".$value->jam_keluar, 1, 0, 'C');
					$pdf->cell(50, 7, $value->jam_lembur_masuk." - ".$value->jam_lembur_keluar, 1, 1, 'C');	
					$no++;
				}
	...
	public function lap2(){
		if(isset($_POST['lap2_submit'])){			
			...
			if(isset($_POST['cetak_lap2_submit'])){
				...
				$pdf->SetFont('Arial','',18);
				$pdf->cell(40, 7, '', 0, 0, 'C');
				$pdf->Cell(195,6,'LAPORAN ABSENSI Per BULAN',0,1,'R');
				...
				$pdf->cell(5, 7, ':', 0, 0, 'C');
				$pdf->cell(140, 7, $bata['nama_karyawan'], 0, 1, 'L');

				$pdf->cell(40, 7, '', 0, 0, 'C');
				$pdf->cell(40, 7, 'Bulan', 0, 0, 'L');
				$pdf->cell(5, 7, ':', 0, 0, 'C');
				$pdf->cell(140, 7, $kata['bln']." ".$kata['thn'], 0, 1, 'L');
				...
				foreach ($data->result() as $value) {
					$pdf->cell(40, 7, '', 0, 0, 'C');
					$pdf->cell(15, 7, $no, 1, 0, 'C');
					$pdf->cell(80, 7, $value->hari.", ".$value->tgl." ".$value->bln." ".$value->thn, 1, 0);
					$pdf->cell(50, 7, $value->jam_masuk." - ".$value->jam_keluar, 1, 0, 'C');
					$pdf->cell(50, 7, $value->jam_lembur_masuk." - ".$value->jam_lembur_keluar, 1, 1, 'C');
					$no++;
				}
